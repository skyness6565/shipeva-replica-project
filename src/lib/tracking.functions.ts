...
export const trackPackage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: pkg, error } = await supabaseAdmin
      .from("packages")
      .select(
        "id,tracking_number,reference_id,package_name,sender_name,sender_email,receiver_name,receiver_email,origin_country,destination_country,current_location,status,shipment_method,package_type,weight,shipment_fee,currency,estimated_delivery_days,shipment_description,dispatch_date,expected_delivery_date,image_urls,customs_hold,created_at,updated_at",
      )
